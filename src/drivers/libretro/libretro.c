...
#define MAX_PLAYERS 12 /* max supported players */
#define MAX_PORTS 12   /* max controller ports,
...
#define NES_8_7_PAR  ((width * (16.0 / 15.0)) / height)
...
#else
static bool overscan_h = false;
static bool overscan_v = true;
#endif
...
void update_nes_controllers(unsigned port, unsigned device)
{
   ...
   if (port < 12)
   {
      ...
      default:
         ...
         if(port<4)FCEUI_SetInput(port, SI_GAMEPAD, &JSReturn[0], 0);
		 else 
			 if(port<8)FCEUI_SetInput(port, SI_GAMEPAD, &JSReturn[1], 0);
		 else 
			 if(port<12)FCEUI_SetInput(port, SI_GAMEPAD, &JSReturn[2], 0);
...
void retro_set_controller_port_device(unsigned port, unsigned device)
{
  ...
  if ((port < 12) && (input_type[port] != device))
   {
     // switch(device)
      ...
      else
      {
         ...
         else /* do famicom controllers here */
         {
            if (device != RETRO_DEVICE_FC_AUTO)
               update_nes_controllers(12, device);
            else
               update_nes_controllers(12, fc_to_libretro(GameInfo->inputfc));
         }
         ...
         if (input_type[12] == RETRO_DEVICE_FC_4PLAYERS)
            FCEUI_DisableFourScore(1);
      }
   }
}
...
void retro_set_environment(retro_environment_t cb)
{
   static const struct retro_variable vars[] = {
...
#else
      { "fceumm_overscan_h", "Crop Overscan (Horizontal); disabled|disabled" },
      ...
      { "fceumm_turbo_enable", "Turbo Enable; Both|Player 1|Player 2|None" },
      { "fceumm_turbo_delay", "Turbo Delay (in frames); 3|5|10|15|30|60|2|1" },
      { "fceumm_zapper_mode", "Zapper Mode; lightgun|mouse" },
      { "fceumm_show_crosshair", "Show Crosshair; enabled|disabled" },
      { "fceumm_overclocking", "Overclocking; disabled|2x-Postrender|2x-VBlank|3x-Postrender|3x-VBlank|4x-Postrender|8x-Postrender" },
	  { "fceumm_exp", "Expansion Port; None|4player" },
      { NULL, NULL },
   };
   ...
   static const struct retro_controller_description pads4[] = {
      ...
      static const struct retro_controller_description pads6[] = {
      { "Auto",     RETRO_DEVICE_AUTO },
      { "Gamepad",  RETRO_DEVICE_GAMEPAD },
      { 0, 0 },
   };
      static const struct retro_controller_description pads7[] = {
      { "Auto",     RETRO_DEVICE_AUTO },
      { "Gamepad",  RETRO_DEVICE_GAMEPAD },
      { 0, 0 },
   };
      static const struct retro_controller_description pads8[] = {
      { "Auto",     RETRO_DEVICE_AUTO },
      { "Gamepad",  RETRO_DEVICE_GAMEPAD },
      { 0, 0 },
   };
      static const struct retro_controller_description pads9[] = {
      { "Auto",     RETRO_DEVICE_AUTO },
      { "Gamepad",  RETRO_DEVICE_GAMEPAD },
      { 0, 0 },
   };
      static const struct retro_controller_description pads10[] = {
      { "Auto",     RETRO_DEVICE_AUTO },
      { "Gamepad",  RETRO_DEVICE_GAMEPAD },
      { 0, 0 },
   };
      static const struct retro_controller_description pads11[] = {
      { "Auto",     RETRO_DEVICE_AUTO },
      { "Gamepad",  RETRO_DEVICE_GAMEPAD },
      { 0, 0 },
   };
         static const struct retro_controller_description pads12[] = {
      { "Auto",     RETRO_DEVICE_AUTO },
      { "Gamepad",  RETRO_DEVICE_GAMEPAD },
      { 0, 0 },
   };
   static const struct retro_controller_description pads13[] = {
...
void retro_get_system_av_info(struct retro_system_av_info *info)
{
...
#else
    unsigned width  = wss - (overscan_h ? 16 : 0);
   ...
   info->geometry.aspect_ratio = (float)(NES_8_7_PAR);
...
//      for(int i=2; i<12; i++) input_type[i] = RETRO_DEVICE_GAMEPAD;
//}
...
void set_input_famicom_controller(int x)
{
   ...
   {
      case SIFC_ARKANOID:
         input_type[12] = RETRO_DEVICE_FC_ARKANOID;
         FCEUI_SetInputFC(0,SIFC_ARKANOID, fc_MouseData, 0);
         FCEU_printf("Famicom Expansion: Arkanoid\n");
         break;
      case SIFC_SHADOW:
         input_type[12] = RETRO_DEVICE_FC_SHADOW;
         FCEUI_SetInputFC(0,SIFC_SHADOW, fc_MouseData, 1);
         FCEU_printf("Famicom Expansion: (Bandai) Hyper Shot\n");
         break;
      case SIFC_OEKAKIDS:
         input_type[12] = RETRO_DEVICE_FC_OEKAKIDS;
         FCEUI_SetInputFC(0,SIFC_OEKAKIDS, fc_MouseData, 1);
         FCEU_printf("Famicom Expansion: Oeka Kids Tablet\n");
         break;
      default:
         ...
         {
            input_type[12] = 0;
            FCEUI_SetInputFC(0,SIFC_NONE, NULL, 0);
            break;
         }
   }
}
...
static void check_variables(bool startup)
{
   ...
   {
      if (!strcmp(var.value, "4player"))
	  {
		  
		 FCEUI_SetInputFC(0, SIFC_4PLAYER, &JSReturn[0], 0);
		 FCEUI_SetInputFC(1, SIFC_4PLAYER, &JSReturn[1], 0);
		 FCEUI_SetInputFC(2, SIFC_4PLAYER, &JSReturn[2], 0);
         enable_4player = true;
	  }
   ...
   if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value)
   {
	 ...
	 else if (!strcmp(var.value, "3x-Postrender"))
      {
         overclocked            = 1;
         skip_7bit_overclocking = 1;
         extrascanlines         = 266*2;
         vblankscanlines        = 0;
         overclock_state        = 1;
         do_reinit              = true;
      }
	  ...
	  else if (!strcmp(var.value, "3x-VBlank"))
      {
         overclocked            = 1;
         skip_7bit_overclocking = 1;
         extrascanlines         = 0;
         vblankscanlines        = 266*2;
         overclock_state        = 1;
         do_reinit              = true;
      }
	  	 else if (!strcmp(var.value, "4x-Postrender"))
      {
         overclocked            = 1;
         skip_7bit_overclocking = 1;
         extrascanlines         = 266*3;
         vblankscanlines        = 0;
         overclock_state        = 1;
         do_reinit              = true;
      }
	  	  	 else if (!strcmp(var.value, "8x-Postrender"))
      {
         overclocked            = 1;
         skip_7bit_overclocking = 1;
         extrascanlines         = 266*7;
         vblankscanlines        = 0;
         overclock_state        = 1;
         do_reinit              = true;
      }
...
JSReturn[2] = 0;
   ...
   for (player = 0; player < MAX_PLAYERS; player++)
   {
      ...
      */
      ...
      if(player<4)JSReturn[0] |= (input_buf & 0xff) << (player << 3);
	  else
	  if(player<8)JSReturn[1] |= (input_buf & 0xff) << ((player-4) << 3);
	  else
	  if(player<12)JSReturn[2] |= (input_buf & 0xff) << ((player-8) << 3);
...
static void retro_run_blit(uint8_t *gfx)
{
   ...
   texture_vram_p = (void*) (0x44200000 - (freebuf * 256)); /* max VRAM address - frame size */

   sceKernelDcacheWritebackRange(retro_palette,256 * 2);
   sceKernelDcacheWritebackRange(XBuf, 512*240 );
  ...
  height -= (overscan_v ? 16 : 0); //it's stupid overscan///
 pitch  -= (overscan_h ? 32 : 0);
  gfx    += (overscan_v ? ((overscan_h ? 8 : 0) + wss * 8) : (overscan_h ? 8 : 0));
...
bool retro_load_game(const struct retro_game_info *game)
{
   ...
   struct retro_input_descriptor desc[] = {
      ...
      { 4, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_X,     "Turbo A" },
      { 4, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_Y,     "Turbo B" },
      ...
      { 5, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_X,     "Turbo A" },
      { 5, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_Y,     "Turbo B" },
      ...
      { 6, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_X,     "Turbo A" },
      { 6, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_Y,     "Turbo B" },
      ...
      { 7, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_X,     "Turbo A" },
      { 7, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_Y,     "Turbo B" },
      ...
      { 8, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_X,     "Turbo A" },
      { 8, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_Y,     "Turbo B" },
      ...
      { 9, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_X,     "Turbo A" },
      { 9, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_Y,     "Turbo B" },
      ...
      { 10, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_X,     "Turbo A" },
      { 10, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_Y,     "Turbo B" },
      ...
      { 11, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_X,     "Turbo A" },
      { 11, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_Y,     "Turbo B" },
   ...
   check_system_specs();
#if defined(_3DS)
   fceu_video_out = (uint16_t*)linearMemAlign(512 * 240 * sizeof(uint16_t), 128);
#elif !defined(PSP)
   fceu_video_out = (uint16_t*)malloc(512 * 240 * sizeof(uint16_t));
#endif
   ...
   for (i = 0; i < 2; i++)
   {
	//  GameInfo->input[i] = SI_GAMEPAD;
      FCEUI_SetInput(i, SI_GAMEPAD, &JSReturn[0], 0);
   }
